...
# ===== 추가: 문자열 예측을 리스트로 안전 파싱 =====
def parse_prediction_to_list(pred):
    """
    When pred is a string:
      - If in '["A", "B"]' / "['A', 'B']" format, parse with literal_eval
      - If in "[A, B]" format, remove brackets and split by comma
      - If multi-line prediction, split by newline
    If pred is already a list, return as is
    """
    ...
    # If multiple newlines, split by line first
    # (fallback for non-list notation)
    if "\n" in s and not (s.startswith("[") and s.endswith("]")):
        cand = [x.strip() for x in s.split("\n") if x.strip()]
        if len(cand) > 1:
            return cand

    # Handle bracket-wrapped notation
    if s.startswith("[") and s.endswith("]"):
        # 1) Try literal_eval first: strong for ["A","B"] or ['A','B'] format
        try:
            maybe_list = ast.literal_eval(s)
            # Convert to string even if numbers/other types are present
            if isinstance(maybe_list, list):
                return [str(x).strip() for x in maybe_list if str(x).strip()]
        except Exception:
            pass

        # 2) Manual parsing if literal_eval fails: [A, B] format
        inner = s[1:-1].strip()
        if inner == "":
            return []
        # Split by comma
        parts = [p.strip() for p in inner.split(",")]
        # Remove quotes and whitespace from both ends
        cleaned = []
        for p in parts:
            # Remove quotes
    ...
    # 3) Final fallback: return as single string item
    return [s]
        ...
        for line in f:
            ...
            if cal_f1:
                if not isinstance(prediction, list):
                    # Safe parsing: properly handles ["A", "B"] format
                    prediction = parse_prediction_to_list(str(prediction))
                else:
